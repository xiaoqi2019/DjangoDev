	...
	def get(self, request):
		...
		user_info = {'username': user.username,
			...
			'date_joined': user.date_joined.strftime(
				'%Y-%m-%d %H:%M:%S') if user.date_joined else '',
		...
		if run_testcases_total_count:
			success_rate = int(
				(run_testcases_success_count / run_testcases_total_count) * 100)
		...
		return Response(data={'user': user_info, 'statistics': statistics})
